...
async function deploySuperRareAndStartAuction(
  artistSigner,
  nftContract,
  tokenId,
  reservePrice,
) {
  const marketplaceSettings = await deploy('MarketplaceSettings');
  const creatorRegistry = await deploy('CreatorRegistry', [[]]);
  const royaltyRegistry = await deploy('RoyaltyRegistry', [creatorRegistry.address]);
  const superRareAuctionHouse = await deploy(
    'SuperRareAuctionHouse',
    [marketplaceSettings.address, royaltyRegistry.address]
  );

  const marketWrapper = await deploy(
    'SuperRareMarketWrapper',
...
async function deployTestContractSetup(
  ...
  if (marketName == MARKET_NAMES.FOUNDATION) {
    ...
    marketContracts = await deploySuperRareAndStartAuction(
      artistSigner,
      nftContract,
      tokenId,
      reservePrice
