...
describe('TokenGating', async () => {
  MARKETS.map((marketName) => {
    ...
    describe(`Case ${i}`, async () => {
      ...
      let partyBid, signer, nftContract, gatedERC20, tokenVault, market, auctionId;
      const { splitRecipient, splitBasisPoints, contributions, auctionReservePrice, bids, claims } = testCase;
      const tokenId = 95;
      ...
      for (let contribution of contributions) {
        ...
        it('Does not accept contribution from non-token holder', async () => {
          ...
          await expect(contribute(partyBid, signer, eth(amount))).to.be.revertedWith("Party::contribute: must hold tokens to contribute");
        });
